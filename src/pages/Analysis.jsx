...
export default function Analysis({ filteredTrades, dateLabel, acctLabel, dateRange, account, tosData, setTosData }) {
  ...
  const [activeTab, setActiveTab]             = useState('calendar'); // 'calendar' | 'charts' | 'aichat'
  ...
  const handleImport = useCallback(async (parsed) => {
    ...
    if (month === 'unknown' && taggedTrips.length) {
      const last = taggedTrips[taggedTrips.length - 1];
...
${tosContext}`;
  ...
  const filteredEquity = useMemo(() => {
    ...
    if (!account || account === 'both') {
      const byDate = {};
      rows.forEach(e => {
        if (!byDate[e.date]) byDate[e.date] = { date: e.date, balance: 0 };
        byDate[e.date].balance += e.balance;
      });
      const parseEqDate2 = (s) => { try { if (s.includes('-')) return new Date(s); const p = s.split('/'); const yr = p[2].length===2?'20'+p[2]:p[2]; return new Date(`${yr}-${p[0].padStart(2,'0')}-${p[1].padStart(2,'0')}`); } catch { return new Date(0); } };
      return Object.values(byDate).sort((a,b) => parseEqDate2(a.date) - parseEqDate2(b.date));
    }
    return rows;
  }, [tosData, account, dateRange?.start?.getTime(), dateRange?.end?.getTime()]);
        ...
        <div style={{ display: 'flex', gap: 4, background: '#111', border: '1px solid #222', borderRadius: 8, padding: 4 }}>
          <button
            ...
            style={{
              ...
              background: activeTab === 'calendar' ? '#1a1a2a' : 'transparent',
        ...
        <div>
          {!tosData && (
            <div style={{ textAlign: 'center', padding: '40px 20px', color: '#444', fontSize: 13 }}>
              Upload a TOS account statement above to see the calendar
            </div>
          )}
          {tosData && (
            <>
              ...
              {(() => {
                ...
                return (
                  <div style={{ background:'#111', border:'1px solid #222', borderRadius:8, padding:'14px 16px', marginBottom:12 }}>
                    <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom: rows.length ? 14 : 0 }}>
                      ...
                      <div style={{ fontSize:11, color:'#555' }}>{tagged.length}/{filteredTrips.length} tagged {untagged.length>0 && <span style={{color:'#f59e0b', marginLeft:8}}>⚠ {untagged.length} untagged — click a day to tag</span>}</div>
                    </div>
                    {rows.length === 0 ? (
                      <div style={{ fontSize:12, color:'#444', paddingTop:8 }}>No strategies tagged yet — click any day cell below to assign strategies</div>
                    ) : (<>
                          ...
                          return (
                            <div key={sid} style={{ background:'#0d0d0d', border:`1px solid ${color}33`, borderTop:`2px solid ${color}`, borderRadius:6, padding:'10px 12px' }}>
                              ...
                              {d ? (<>
                                ...
                                <div style={{ fontSize:11, color:'#555' }}>{d.trades}t · {Math.round(d.wins/d.trades*100)}% WR</div>
                      ...
                      <div>{rows.map(([sid,d]) => {
                        ...
                        return (
                          <div key={sid} style={{ display:'flex', alignItems:'center', gap:10, marginBottom:6 }}>
                            ...
                            <div style={{ flex:1, height:20, background:'#0d0d0d', borderRadius:4, overflow:'hidden' }}>
                ...
                return (
                  <div style={{ background:'#111', border:'1px solid #222', borderRadius:8, padding:'14px 16px', marginTop:12 }}>
                    <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:14 }}>
                      <div style={{ fontSize:11, color:'#bbb', textTransform:'uppercase', letterSpacing:'0.05em', fontWeight:700 }}>By Strategy — TOS Broker Data</div>
                      <div style={{ fontSize:11, color:'#555' }}>{tagged.length}/{filteredTrips.length} tagged {untagged.length>0 && <span style={{color:'#f59e0b', marginLeft:8}}>⚠ {untagged.length} untagged</span>}</div>
                    </div>
                    {rows.length === 0 ? (
                      <div style={{ fontSize:12, color:'#444', padding:'20px 0', textAlign:'center' }}>No strategies tagged yet — click a day cell and assign strategies to trades</div>
                    ) : (<>
                      ...
                      <div>
                        ...
                        {rows.map(([sid,d]) => {
                          const color = STRAT_COLORS[sid]||'#666';
                          const barW = Math.abs(d.net)/maxAbs*100;
                          return (
                            <div key={sid} style={{ display:'flex', alignItems:'center', gap:10, marginBottom:8 }}>
                              ...
                              <div style={{ flex:1, height:24, background:'#0d0d0d', borderRadius:4, overflow:'hidden' }}>
                                <div style={{ width:barW+'%', height:'100%', background:d.net>=0?'#1D9E75':'#E24B4A', borderRadius:4, opacity:0.85 }} />
                              </div>
                              <div style={{ width:80, fontSize:12, fontWeight:700, color:d.net>=0?'#1D9E75':'#E24B4A', flexShrink:0 }}>{fmtP(d.net)}</div>
                              <div style={{ width:60, fontSize:11, color:'#555', flexShrink:0 }}>{Math.round(d.wins/d.trades*100)}% WR</div>
                            </div>
                          );
                        })}
                      </div>
